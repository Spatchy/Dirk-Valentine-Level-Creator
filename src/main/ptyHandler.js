...
export default {
  ...
  useSwfextract() {
    const location = settingsManager.getSetting("swftoolsPath");
    this.setPtyLocation(location);
    ...
    let id;
    let fileExt;
    const cmdString = `./swfextract -${letter} ${id} "${settingsManager.appdata}/projects/dirkvalentine.swf" -o "${extractFolder}/${id}.${fileExt}"`;
    ...
    letter = "p";
    fileExt = "png";
    pngIds.forEach(element => {
      id = element;
      ptyProcess.write(cmdString);
    });
    letter = "j";
    fileExt = "jpeg";
    jpegIds.forEach(element => {
      id = element;
      ptyProcess.write(cmdString);
    });
  }
